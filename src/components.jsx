...
import { 
  MapPin, ExternalLink, Bookmark, Users, Landmark, 
  Compass, Search, Menu, X, Globe, Layers, AlertTriangle 
} from 'lucide-react';

// ---------- AttractionCard ----------
export function AttractionCard({ title, extract, thumbnail, distanceMeters, coords }) {
  return (
    <a
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-2">
          <h4 className="font-display text-base text-parchment-50 leading-snug">{title}</h4>
          <ExternalLink className="h-3.5 w-3.5 shrink-0 mt-1 text-ink-400 group-hover:text-brass-300 transition-colors" />
        </div>
        {extract && <p className="text-sm text-ink-300 mt-1 line-clamp-2">{extract}</p>}
        <div className="flex items-center gap-3 mt-2">
          {distanceMeters != null && (
            <span className="coord">{(distanceMeters / 1000).toFixed(1)} km away</span>
          )}
        </div>
      </div>
    </a>
...
export function CountryCard({ country, index = 0 }) {
  ...
  return (
    <Link to={`/country/${country.cca3}`} className="card-stamp group block h-full">
      <div className="relative aspect-[4/3] overflow-hidden bg-ink-700">
        ...
        <button
          ...
          className={`absolute top-3 right-3 flex h-9 w-9 items-center justify-center rounded-full backdrop-blur-md transition-all ${
            saved ? 'bg-brass-300 text-ink-900' : 'bg-ink-900/60 text-parchment-100 hover:bg-ink-900/80'
          }`}
        >
      ...
      <div className="p-4 flex flex-wrap items-center gap-x-4 gap-y-2 justify-between">
        ...
        <span className="text-xs font-mono text-teal-300 truncate">{country.region}</span>
      </div>
    </Link>
  );
}
...
// ---------- FilterChips ----------
export function FilterChips({ options, value, onChange, label = 'Filter by region' }) {
  return (
  ...
  return (
    <footer className="border-t border-ink-700/80 mt-12 sm:mt-24">
      <div className="container-wide py-10 sm:py-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-10">
        <div className="sm:col-span-2 lg:col-span-1">
          <div className="flex items-center gap-2 mb-3">
            <Compass className="h-5 w-5 text-brass-300 shrink-0" strokeWidth={1.75} />
            <span className="font-display text-lg text-parchment-50">Atlasora</span>
          </div>
          <p className="text-sm text-ink-300 leading-relaxed max-w-sm">
            A field guide to everywhere. Discover, chart, and save the places worth the trip.
          </p>
        </div>
      ...
      <div className="border-t border-ink-700/80">
        <div className="container-wide py-4 sm:py-5 flex flex-col sm:flex-row items-center justify-between gap-2 text-[10px] sm:text-xs text-ink-400 font-mono text-center sm:text-left">
          <span>© {new Date().getFullYear()} ATLASORA · COORDINATES SUBJECT TO CHANGE</span>
  ...
  return (
    <div>
      ...
      <ul className="space-y-2">
        {links.map((l) => (
          <li key={l.label}>
            <Link to={l.to} className="text-sm text-ink-300 hover:text-brass-200 transition-colors">
              {l.label}
            </Link>
          </li>
        ))}
      ...
      <div className="container-wide flex h-16 sm:h-20 lg:h-24 items-center justify-between gap-4">
        <Link to="/" className="flex items-center gap-2 sm:gap-3 shrink-0 group min-w-0">
          <div className="relative shrink-0">
            ...
            <Compass className="h-7 w-7 sm:h-8 sm:w-8 text-brass-300 transition-transform duration-700 group-hover:rotate-180 relative" strokeWidth={1.5} />
          </div>
          <div className="flex flex-col min-w-0">
            <span className="font-display text-xl sm:text-2xl tracking-tight text-parchment-50 leading-none truncate">Atlasora</span>
            <span className="font-mono text-[9px] sm:text-[10px] tracking-[0.25em] sm:tracking-[0.3em] text-brass-300/70 uppercase hidden sm:block">Global Intelligence</span>
          </div>
        </Link>
        ...
        <nav className="hidden lg:flex items-center gap-1" aria-label="Main navigation">
          {navLinks.map((l) => (
            ...
            >
              <l.icon className="h-4 w-4 transition-transform group-hover:scale-110" strokeWidth={1.5} />
              {l.label}
            </NavLink>
          ))}
        </nav>
        ...
        <div className="hidden lg:flex items-center gap-3">
          <Link to="/explore" className="group flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium text-parchment-300 hover:text-parchment-50 hover:bg-ink-800/50 border border-transparent hover:border-ink-600 transition-all duration-300">
            <Search className="h-4 w-4 transition-transform group-hover:scale-110" strokeWidth={1.5} />
            Search
          </Link>
          <Link to="/saved" className="group flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium text-parchment-300 hover:text-parchment-50 hover:bg-ink-800/50 border border-transparent hover:border-ink-600 transition-all duration-300">
            <Bookmark className="h-4 w-4 transition-transform group-hover:scale-110" strokeWidth={1.5} />
            Saved
          </Link>
        </div>
...
// ---------- SearchBar ----------
export function SearchBar({ value, onChange, placeholder = 'Search a country, capital, or region…', large = false, className = '' }) {
  return (
...
export function CenteredSpinner({ label = 'Charting the map…' }) {
  return (
    <div className="flex flex-col items-center justify-center py-24 gap-4">
      <Compass className="h-8 w-8 text-brass-300 animate-spin-slow" strokeWidth={1.5} />
      <p className="coord">{label}</p>
    </div>
  );
}

export function ErrorState({ message = 'Something drifted off course.', onRetry }) {
  return (
...
export function EmptyState({ icon: Icon = Compass, title, description, action }) {
  return (
    <div className="flex flex-col items-center justify-center py-24 gap-4 text-center px-6">
      ...
      {description && <p className="text-ink-300 max-w-sm text-sm">{description}</p>}
      {action}
    </div>
  );
}
...
export function useSaved() {
  throw new Error('useSaved must be used within SavedProvider - import from context.jsx');
}

export function useAuth() {
  throw new Error('useAuth must be used within AuthProvider - import from context.jsx');
}
